...
public class LocationService extends Service implements
    ...
    protected void sendLocationDataToWebsite(Location location) {
        ...
        SharedPreferences sharedPreferences = this.getSharedPreferences("com.tryagain.com.fleetmanagmentsystem", Context.MODE_PRIVATE);
        ...
        String oldDateTime = sharedPreferences.getString("datetime", "");
        Log.i("oldtime", oldDateTime);
        String newDateTime = dateFormat.format(date);
        float distance = 0;
        if (firstTimeGettingPosition) {
            editor.putBoolean("firstTimeGettingPosition", false);
            //editor.putFloat("previousLatitude"location.getLatitude());
            //editor.putFloat("previousLongitude", (flo, (float) at) location.getLongitude());
            //editor.putString("datetime", newDateTime);
        } else {
        ...
        float lt = sharedPreferences.getFloat("start_latitude", 0f);

        if(lt == 0){
            editor.putFloat("start_latitude", (float) location.getLatitude());
            editor.putFloat("start_longitude", (float) location.getLongitude());
        }

        editor.putFloat("end_latitude", (float) location.getLatitude());
        editor.putFloat("end_longitude", (float) location.getLongitude());
        ...
        float speedInMeterPerSecond = 0;
        /*
        long timeDifference = 0;
        try {
            timeDifference = getDateDiff(dateFormat.parse(oldDateTime), dateFormat.parse(newDateTime), TimeUnit.SECONDS);
        } catch (Exception ex){
            Log.e("Error", ex.toString());
            return;
        }
        Log.i("time", timeDifference + "");
        float speedInMeterPerSecond = distance / (float)timeDifference;
        if (speedInMeterPerSecond > 0.2) {
            playSound();
        }
        */
        requestParams.put("speed", location.getSpeed() + "");
        String stringDate = dateFormat.format(date) + "";
        stringDate = stringDate.replace(" " , "tag1");
        stringDate = stringDate.replace(":" , "tag2");

        Log.e("datetag", stringDate);
        requestParams.put("date", stringDate);
        /*
        try {
        ...
        */
        ...
        if (totalDistanceInMeters > 0) {
            requestParams.put("distance", String.format("%.1f", totalDistanceInMeters / 1000)); // in kms,
        } else {
            requestParams.put("distance", "0.0"); // in kms
        }
